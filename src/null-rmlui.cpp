...
namespace ntl::rml {
    ...
    void initialize() {
        ...
        else sdk::logger(sdk::e_log_type::error, "file_interface is empty.");

        if(system_interface) Rml::SetSystemInterface(system_interface.get());
        else sdk::logger(sdk::e_log_type::error, "system_interface is empty.");
        ...
        if(!Rml::Initialise()) sdk::logger(sdk::e_log_type::error, "Rml::Initialise return false.");
    }
