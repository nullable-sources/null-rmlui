...
namespace ntl::rml::renderer {
    ...
    class i_geometry_filter {
    public:
        virtual void render(Rml::CompiledGeometryHandle geometry_handle, const Rml::Vector2f& translation, Rml::TextureHandle texture) = 0;
    };

    class i_filter_instancer {
    public:
        virtual std::unique_ptr<i_filter> instance_filter(const Rml::Dictionary& parameters) = 0;
    };

    class i_geometry_filter_instancer {
    public:
        virtual std::unique_ptr<i_geometry_filter> instance_filter(const Rml::Dictionary& parameters) = 0;
    };
